...
export default function AdminLayout({
  ...
  const handleLogout = () => {
    // 1. Xóa sạch token xác thực và data user lưu ở trình duyệt
    Cookies.remove('token');
    Cookies.remove('user');

    // 2. Đá người dùng quay về trang đăng nhập
    ...
    <div className="flex min-h-screen bg-[#120c0a] text-[#f4f1ea] font-sans antialiased">
      ...
      <aside className="w-72 border-r border-[#2d221e] bg-[#1c1412]/95 backdrop-blur-md p-6 flex flex-col justify-between sticky top-0 h-screen z-50">
        <div className="space-y-8">
          ...
          {/* Khối Thông tin tổng quan nhanh */}
          <div className="p-4 rounded-xl bg-[#281d1a]/60 border border-[#3d2d28]/40 space-y-2">
            <div className="text-xs text-[#a3968e]">Trạng thái máy pha:</div>
