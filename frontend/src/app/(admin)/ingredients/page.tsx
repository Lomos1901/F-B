...
// --- MODAL COMPONENTS (Cập nhật giao diện sáng) ---
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-white border border-gray-100 p-6 rounded-2xl shadow-xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        <h2 className="text-xl font-bold mb-4 text-[#4B2C20] flex items-center gap-2"><Inbox className="text-[#FFB800]" size={24}/>Nhập kho nguyên liệu</h2>
        <div className="p-4 bg-[#FCF9F8] rounded-xl mb-4 border border-[#FFB800]/20">
          <p className="font-semibold text-[#4B2C20]">{ingredient.name}</p>
          <p className="text-sm text-gray-500 mt-1">Tồn hiện tại: <span className="font-bold text-[#FFB800]">{ingredient.stock_quantity} {ingredient.base_unit}</span></p>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Số lượng nhập ({ingredient.base_unit}) <span className="text-red-500">*</span></label>
            <input type="number" placeholder="Nhập số lượng..." value={amount} onChange={e => setAmount(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#FFB800] focus:border-[#FFB800] outline-none transition-all" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Ghi chú</label>
            <input type="text" placeholder="Ví dụ: NCC ABC giao..." value={note} onChange={e => setNote(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#FFB800] focus:border-[#FFB800] outline-none transition-all" />
          </div>
        </div>
        <div className="flex justify-end mt-8 space-x-3">
          ...
          <button onClick={handleSubmit} disabled={isSubmitting} className="px-5 py-2.5 bg-[#FFB800] text-white rounded-xl font-bold hover:bg-[#F0AD00] transition-colors flex items-center gap-2">
...
const StocktakeModal = ({ ingredient, onClose, onConfirm }: StocktakeModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-white border border-gray-100 p-6 rounded-2xl shadow-xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        <h2 className="text-xl font-bold mb-4 text-[#4B2C20] flex items-center gap-2"><ClipboardList className="text-blue-500" size={24}/>Kiểm kê kho</h2>
        <div className="p-4 bg-blue-50 rounded-xl mb-4 border border-blue-100">
          <p className="font-semibold text-[#4B2C20]">{ingredient.name}</p>
          <p className="text-sm text-gray-600 mt-1">Tồn trên hệ thống: <span className="font-bold text-blue-600">{ingredient.stock_quantity} {ingredient.base_unit}</span></p>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Số lượng thực tế ({ingredient.base_unit}) <span className="text-red-500">*</span></label>
            <input type="number" placeholder="Đếm được bao nhiêu..." value={actualQuantity} onChange={e => setActualQuantity(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
            {actualQuantity !== '' && (
              <p className={`text-xs mt-1.5 font-bold ${Number(actualQuantity) - ingredient.stock_quantity > 0 ? 'text-green-600' : Number(actualQuantity) - ingredient.stock_quantity < 0 ? 'text-red-600' : 'text-gray-500'}`}>
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Lý do kiểm kho <span className="text-red-500">*</span></label>
            <input type="text" placeholder="Ví dụ: Hao hụt, Hư hỏng..." value={note} onChange={e => setNote(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
          </div>
        </div>
        <div className="flex justify-end mt-8 space-x-3">
          <button onClick={onClose} disabled={isSubmitting} className="px-5 py-2.5 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 font-bold transition-colors">Hủy bỏ</button>
          <button onClick={handleSubmit} disabled={isSubmitting} className="px-5 py-2.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-colors flex items-center gap-2">
            {isSubmitting && <Loader2 size={16} className="animate-spin" />}
            Xác nhận Kiểm
          </button>
        </div>
...
const DeleteModal = ({ ingredient, usage, onClose, onConfirm, onHardConfirm }: DeleteModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-white border border-gray-100 p-6 rounded-2xl shadow-xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        <h2 className="text-xl font-bold mb-4 text-red-600 flex items-center gap-2"><AlertTriangle size={24}/>Xác nhận xóa nguyên liệu</h2>
        <p className="text-gray-600">Bạn có chắc muốn xóa nguyên liệu <strong>{ingredient.name}</strong> không? Hành động này có thể ảnh hưởng đến lịch sử thống kê.</p>
        ...
        <div className="flex justify-end mt-8 space-x-3">
          <button onClick={onClose} disabled={isSubmitting} className="px-5 py-2.5 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 font-bold transition-colors">Hủy bỏ</button>
          {!usage.is_used && (
            <button onClick={() => handleAction(onHardConfirm)} disabled={isSubmitting} className="px-5 py-2.5 bg-gray-800 text-white rounded-xl font-bold hover:bg-gray-900 transition-colors">Xóa vĩnh viễn</button>
          )}
          <button onClick={() => handleAction(onConfirm)} disabled={isSubmitting} className="px-5 py-2.5 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-colors flex items-center gap-2">
            {isSubmitting && <Loader2 size={16} className="animate-spin" />}
...
export default function IngredientsPage() {
  ...
  if (error && ingredients.length === 0) {
    return <div className="p-8 text-red-500 font-bold bg-[#FCF9F8] h-full flex items-center justify-center">{error}</div>;
  }
    ...
    <main className="flex flex-col h-full bg-[#FCF9F8] text-[#4B2C20]" style={{ fontFamily: 'Montserrat, sans-serif' }}>
      ...
      <div className="px-6 py-6 border-b border-gray-200 bg-white flex flex-col xl:flex-row xl:justify-between xl:items-center gap-4 sticky top-0 z-10 shadow-sm">
        <div>
          ...
          <p className="text-gray-500 mt-1">Quản lý tồn kho, nhập hàng và cảnh báo hết hàng.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#FFB800] focus:border-[#FFB800] outline-none text-sm font-semibold w-60"
            />
          </div>
          ...
          <Link href="/ingredients/archived" className="p-2.5 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-100 transition-colors" title="Thùng rác">
            <Archive size={20} />
          </Link>

          <Link href="/ingredients/create" className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-sm bg-[#FFB800] text-white hover:bg-[#F0AD00] transition-colors shadow-sm shadow-amber-200">
            <Plus size={18} />
            Thêm mới
          </Link>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="text-xs uppercase text-gray-400 bg-gray-50 border-b border-gray-200">
                  <th className="px-6 py-4 font-bold">Nguyên liệu</th>
                  <th className="px-6 py-4 font-bold">Danh mục</th>
                  <th className="px-6 py-4 font-bold">Tồn (Nhập)</th>
                  <th className="px-6 py-4 font-bold">Tồn (Pha chế)</th>
                  <th className="px-6 py-4 font-bold text-right">Giá vốn</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredIngredients.length === 0 ? (
                  ...
                  filteredIngredients.map((ing) => {
                    ...
                    return (
                      <tr key={ing.id} className={`hover:bg-gray-50/50 transition-colors ${isLowStock ? 'bg-red-50/30' : ''}`}>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-[#4B2C20]">{ing.name}</span>
                            {isLowStock && (
                        ...
                        <td className="px-6 py-4">
                          <span className={`font-mono font-bold px-2 py-1 rounded-lg text-sm ${isLowStock ? 'text-red-600 bg-red-100' : 'text-[#4B2C20] bg-gray-100'}`}>
                            {ing.stock_quantity.toLocaleString('vi-VN')} {ing.base_unit}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`font-mono font-bold text-sm ${isLowStock ? 'text-red-500' : 'text-gray-500'}`}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-center gap-1.5">
                            <button onClick={() => setModal({ type: 'import', data: ing })} className="p-2 text-green-600 hover:bg-green-50 rounded-xl transition-colors" title="Nhập kho">
                              <Inbox size={18} />
                            </button>
                            ...
                            <Link href={`/ingredients/edit/${ing.id}`} className="p-2 text-amber-600 hover:bg-amber-50 rounded-xl transition-colors" title="Chỉnh sửa">
                              <Edit size={18} />
                            </Link>
